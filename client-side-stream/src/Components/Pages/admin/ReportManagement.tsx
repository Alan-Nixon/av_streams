            ...
            <Layout>
                <div className="m-2">
                    ...
                    {videoReports.map((item, idx) => {
                        return (<p key={idx} className="flex m-3 flex-col items-center border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl bg-white hover:bg-gray-300">
                            ...
                            <div className="flex flex-col justify-between p-4 leading-normal">
                                <h5 className="mb-2 text-lg font-bold tracking-tight text-black">Reported by : {item.channelName}</h5>
                                <p className="mb-3 font-normal text-black">Reason : {item.Reason}</p>
                                {!item.Responded && <button type="button" onClick={() => blockContent(item.LinkId, item.Section, item._id)} style={{ width: "150px" }} className="py-2.5 w-full px-5 me-2 mb-2 text-sm font-medium rounded-lg border bg-gray-800 text-gray-400 border-gray-600 hover:text-white hover:bg-gray-700">Block visiblity</button>}
                    ...
                    <div className="mt-5">
                        ...
                        {blockedVideos.length !==0 ?  blockedVideos.map((item, idx) => {
                            return (
                                <p key={idx} className="flex flex-col items-center border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl bg-white hover:bg-gray-300">
                                    ...
                                    <div className="flex text-black flex-col justify-between ml-3 leading-normal">
                                        ...
                                        <button type="button" onClick={() => changeVisiblity(item._id, "video")} style={{ width: "150px" }} className="py-2.5 w-full px-5 me-2 mb-2 text-sm font-medium rounded-lg border bg-gray-800 text-gray-400 border-gray-600 hover:text-white hover:bg-gray-700">Unblock visiblity</button>
                                    </div>
                                </p>
                            )
