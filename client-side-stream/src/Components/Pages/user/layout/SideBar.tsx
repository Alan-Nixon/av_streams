import { Link, useLocation } from "react-router-dom"
import { useUser } from "../../../../UserContext"
import { useEffect, useState } from "react"
import { channelInterface } from "../../../../Functions/interfaces"
import { getTrendingChannels } from "../../../../Functions/userFunctions/userManagement"
import MissedVideoCallIcon from '@mui/icons-material/MissedVideoCall';
import SettingsOverscanIcon from '@mui/icons-material/SettingsOverscan';

export default function SideBar() {
    const pathName = useLocation()
    const { showHideSideBar, user } = useUser()
    const [currentLives, setCurrentLives] = useState<channelInterface[]>([]);

    useEffect(() => {
        getTrendingChannels(5).then(({ data }) => setCurrentLives(data))
    }, [])

    const sideBarData = [{
        text: "Home",
        Link: "/",
        icon: <>
            <svg className="w-5 h-5 transition duration-75 text-gray-400 group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
                <path d="M16.975 11H10V4.025a1 1 0 0 0-1.066-.998 8.5 8.5 0 1 0 9.039 9.039.999.999 0 0 0-1-1.066h.002Z" />
                <path d="M12.5 0c-.157 0-.311.01-.565.027A1 1 0 0 0 11 1.02V10h8.975a1 1 0 0 0 1-.935c.013-.188.028-.374.028-.565A8.51 8.51 0 0 0 12.5 0Z" />
            </svg>
        </>
    }, {
        text: "Live Now",
        Link: "/liveNow",
        icon: <>
            <svg className="flex-shrink-0 w-5 h-5 transition duration-75 text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 18">
                <path d="M6.143 0H1.857A1.857 1.857 0 0 0 0 1.857v4.286C0 7.169.831 8 1.857 8h4.286A1.857 1.857 0 0 0 8 6.143V1.857A1.857 1.857 0 0 0 6.143 0Zm10 0h-4.286A1.857 1.857 0 0 0 10 1.857v4.286C10 7.169 10.831 8 11.857 8h4.286A1.857 1.857 0 0 0 18 6.143V1.857A1.857 1.857 0 0 0 16.143 0Zm-10 10H1.857A1.857 1.857 0 0 0 0 11.857v4.286C0 17.169.831 18 1.857 18h4.286A1.857 1.857 0 0 0 8 16.143v-4.286A1.857 1.857 0 0 0 6.143 10Zm10 0h-4.286A1.857 1.857 0 0 0 10 11.857v4.286c0 1.026.831 1.857 1.857 1.857h4.286A1.857 1.857 0 0 0 18 16.143v-4.286A1.857 1.857 0 0 0 16.143 10Z" />
            </svg>
        </>
    }, {
        text: "Videos",
        Link: "/videos/",
        icon: <>
            <MissedVideoCallIcon className="text-gray-400 w-5 h-5" />
        </>
    }, {
        text: "Shorts",
        Link: "/shorts",
        icon: <>
            <svg className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                <path d="M14 2a3.963 3.963 0 0 0-1.4.267 6.439 6.439 0 0 1-1.331 6.638A4 4 0 1 0 14 2Zm1 9h-1.264A6.957 6.957 0 0 1 15 15v2a2.97 2.97 0 0 1-.184 1H19a1 1 0 0 0 1-1v-1a5.006 5.006 0 0 0-5-5ZM6.5 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9ZM8 10H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
            </svg>
        </>
    }, {
        text: "Channels",
        Link: "/channels",
        icon: <>
            <svg className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                <path d="M14 2a3.963 3.963 0 0 0-1.4.267 6.439 6.439 0 0 1-1.331 6.638A4 4 0 1 0 14 2Zm1 9h-1.264A6.957 6.957 0 0 1 15 15v2a2.97 2.97 0 0 1-.184 1H19a1 1 0 0 0 1-1v-1a5.006 5.006 0 0 0-5-5ZM6.5 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9ZM8 10H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
            </svg>
        </>
    }]

    return (<>
        <div className="sidebarParentDiv fixed top-[64px] bottom-0 left-0 bg-gray-800 w-50">
            <aside id="default-sidebar" className="fixed top-19 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-900 ">
                    <ul className="space-y-2 font-medium">
                        {sideBarData.map((item) => (<li>
                            <Link to={item.Link} className={`flex items-center p-2 rounded-lg text-white ${pathName.pathname === item.Link ? "bg-gray-700" : "hover:bg-gray-700"} group`}>
                                {item.icon}
                                <span className="ms-3">{item.text}</span>
                            </Link>
                        </li>))}
                        <br /><hr />
                        <span className="flex ms-3 whitespace-nowrap" style={{ marginLeft: "30%", letterSpacing: "1px" }}><strong>Recomanded</strong></span>
                    </ul>
                    <ul style={{ marginLeft: "3%", marginTop: "5%" }}>
                        {currentLives.map((item, index) =>
                            <Link to={`/channel?userId=${item._id}`}>

                                <li className="hover:bg-gray-600 p-1 mt-1" key={index} style={{ display: "flex", borderRadius: "20%" }}>
                                    <img src={item.profileImage} style={{ borderRadius: "100%", width: "35px", height: "35px" }} alt="" />
                                    <div style={{ marginLeft: "10%", margin: "2%", display: "flex" }}>
                                        <p style={{ fontWeight: "400" }} >{item.channelName}</p>

                                    </div>
                                </li>
                            </Link>
                        )}
                    </ul>
                </div>
            </aside>

        </div>
        <div className={`buttonsidebarFromSideBar w-full bg-yellow ${showHideSideBar ? "hidden" : ""}`} style={{ marginTop: "70px" }}>
            <div className="h-full px-3 py-4 overflow-y-autobg-gray-800">
                <ul className="space-y-2 font-medium">
                    {[...sideBarData, {
                        text: user ? "Profile" : "Login",
                        Link: user ? "/profile" : "/login",
                        icon: <>
                            <svg className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                                <path d="M14 2a3.963 3.963 0 0 0-1.4.267 6.439 6.439 0 0 1-1.331 6.638A4 4 0 1 0 14 2Zm1 9h-1.264A6.957 6.957 0 0 1 15 15v2a2.97 2.97 0 0 1-.184 1H19a1 1 0 0 0 1-1v-1a5.006 5.006 0 0 0-5-5ZM6.5 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9ZM8 10H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
                            </svg></>
                    }].map((item) => (<>
                        <li>
                            <Link to={item.Link} className={`flex items-center p-2 rounded-lg text-white ${pathName.pathname === item.Link ? "bg-gray-700" : "hover:bg-gray-700"} group`}>
                                {item.icon}
                                <span className="ms-3">{item.text}</span>
                            </Link>
                        </li>
                    </>))}
                    {user && <>
                        <li>
                            <Link to={"/QrCodeScan"} className={`flex items-center p-2 rounded-lg text-white ${pathName.pathname === "QrCodeScan" ? "bg-gray-700" : "hover:bg-gray-700"} group`}>
                                <SettingsOverscanIcon className="text-gray-400 w-5 h-5" />
                                <span className="ms-3">Scan Qr code</span>
                            </Link>
                        </li>
                    </>}
                    {!user && <>
                        <li>
                            <Link to="/signup" className={`flex items-center p-2 rounded-lg text-white ${"hover:bg-gray-700"} group`}>
                                <svg className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                                    <path d="M14 2a3.963 3.963 0 0 0-1.4.267 6.439 6.439 0 0 1-1.331 6.638A4 4 0 1 0 14 2Zm1 9h-1.264A6.957 6.957 0 0 1 15 15v2a2.97 2.97 0 0 1-.184 1H19a1 1 0 0 0 1-1v-1a5.006 5.006 0 0 0-5-5ZM6.5 9a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9ZM8 10H5a5.006 5.006 0 0 0-5 5v2a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-2a5.006 5.006 0 0 0-5-5Z" />
                                </svg>
                                <span className="ms-3">signup</span>
                            </Link>
                        </li>
                    </>}
                    <br />
                </ul >
            </div>
        </div>
    </>)
}

