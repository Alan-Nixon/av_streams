            ...
            {loading ? <><div className="lds-dual-ring"></div></> : <>
                <div className="flex m-4 ml-5">
                    {["Videos", "Profile"].map((names, idx) => {
                        return (
                            <button key={idx} onClick={() => setCate(names)} type="button" className="ml-2 text-white focus:outline-none focus:ring-4 font-medium rounded-lg text-sm px-5 py-2 me-2 mb-2 bg-gray-800 hover:bg-gray-700 focus:ring-gray-700 border-gray-700">{names}</button>
                        )
                    })}
                </div>
                <div className="m-5">
